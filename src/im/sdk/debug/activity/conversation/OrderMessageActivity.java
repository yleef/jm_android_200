            ...
            public void onClick(View v) {
                ...
                if (mConversation != null) {
                    if (mEt_startGet.getText().toString().length() != 0 && mEt_endGet.getText().toString().length() != 0) {
                        int start = Integer.parseInt(mEt_startGet.getText().toString());
                        int end = Integer.parseInt(mEt_endGet.getText().toString());
                        List<Message> allMessage = null;
                        StringBuilder sb = new StringBuilder();
                        /**=================     会话的message列表排序    =================*/
                        if (mEt_inset.getText().toString().equals("true")) {
                            allMessage = mConversation.getMessagesFromOldest(start, end);
                            orderMessage(sb, allMessage);
                            mTv_show_message_info.setText("");
                            mTv_show_message_info.append("getMessagesFromOldest : " + "\n" + sb.toString() + "\n");
                        } else {
                            allMessage = mConversation.getMessagesFromNewest(start, end);
                            orderMessage(sb, allMessage);
                            mTv_show_message_info.setText("");
