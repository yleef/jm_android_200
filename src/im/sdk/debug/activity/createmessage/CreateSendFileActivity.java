 ...
 *
 * @desc :创建发送文件消息,有两中发送文件消息接口,不同之处在于先创建FileContent就可以通过content获取相关接口功能,
 * 如获取自己填写的文件名;sdk有提供手动下载文件接口,用法演示是A发送文件消息,B会收到通知,然后B点击通知栏会进入到手动
 * 下载界面,点击手动下载,就会将文件下载到B的本地;收到的Event事件请看{@link im.sdk.debug.activity.TypeActivity}
 */
    ...
    private void initData() {
        mBt_getFile.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mFileMp3 = new File(Environment.getExternalStorageDirectory().getPath() + "/voice/test.txt");
                InputStream in = null;
                try {
                    in = getApplicationContext().getAssets().open("test.txt");
        ...
        mBt_send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                File fileMp3 = new File(Environment.getExternalStorageDirectory().getPath() + "/voice/test.txt");
                if (!fileMp3.exists()) {
